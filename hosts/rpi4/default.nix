{ config, pkgs, user, ... }: let buildService = import ../../containers/buildService.nix { pkgs = pkgs; user = user; }; in {
  ...
  systemd.user.services."samba" = (buildService {
    ...
    description = "Samba";
    after = [ "sops.service" ];
    options = "--userns=keep-id:uid=1000,gid=1000 -p 6445:445/tcp --volume /home/${user}/.config/smb_config.yaml:/data/config.yml --volume /home/${user}/data:/samba ghcr.io/crazy-max/samba";
  });
}
